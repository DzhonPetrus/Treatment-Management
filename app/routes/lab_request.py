...
def show(request: Request, id, db: Session = Depends(get_db)):
    ...
    return templates.TemplateResponse("lab_request.html", {"request":request, "lab_request": surgery_type.get_one(id, db)})
...
def all(request: Request, db: Session = Depends(get_db)):
    ...
    return templates.TemplateResponse("lab_request.html", {"request":request, "lab_request": jsonable_encoder(lab_request.get_all(db))})